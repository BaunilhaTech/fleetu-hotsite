...
export function LanguageSwitcher() {
    ...
    const changeLocale = (nextLocale: string) => {
        ...
        if ("startViewTransition" in document) {
            (document as { startViewTransition: (cb: () => void) => void }).startViewTransition(navigate);
        } else {
            navigate();
        }
    };
    ...
    return (
        <div className="relative inline-flex items-center rounded-full border border-primary/25 bg-gradient-to-r from-primary/15 via-background/80 to-primary/10 p-1 shadow-[0_10px_30px_-18px_hsl(var(--primary)/0.7)] backdrop-blur-md">
            <div className="relative grid grid-cols-2 gap-1">
                ...
                <span
                    className={cn(
                        "absolute inset-y-0 w-[calc(50%-2px)] rounded-full border border-primary/30 bg-primary/20 transition-transform duration-200 ease-out",
                        locale === languageOptions[1].code && "translate-x-[calc(100%+4px)]"
                    )}
                />
                {languageOptions.map((option) => {
                    ...
                    return (
                        <button
                            ...
                            className={cn(
                                "relative z-10 min-w-[42px] lg:min-w-[54px] rounded-full px-2 lg:px-2.5 py-1.5 text-xs font-semibold transition-colors duration-200",
